...
# Page Configuration
st.set_page_config(
    page_title="Akshitha Predict+ Dashboard",
    layout="wide",
...
st.markdown('<h1 class="tagline">Akshitha Predict+</h1>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)

# Intro Card
st.markdown("""
<div class="stCard">
    <h4>📊 Welcome to Akshitha Predict+</h4>
    <p>This smart forecasting dashboard allows you to analyze and predict trends from time-based sales data using a custom ARIMA-powered engine.</p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    st.markdown('<h3>📥 Upload Your Dataset</h3>', unsafe_allow_html=True)
    uploaded_file = st.file_uploader("Upload a CSV with time and sales data", type=["csv"])
    st.markdown('</div>', unsafe_allow_html=True)

    if uploaded_file is not None:
        st.markdown('<div class="stCard">', unsafe_allow_html=True)
        st.markdown('<h3>⚙️ Forecasting Parameters</h3>', unsafe_allow_html=True)
        ...
        seasonal_d = st.number_input("Seasonal Diff (D)", min_value=0, max_value=2, value=1)
...
with col2:
    if uploaded_file is not None:
        ...
        try:
            ...
            st.markdown('<div class="stCard">', unsafe_allow_html=True)
            st.markdown('<h3>📈 Sales Trend Visualization</h3>', unsafe_allow_html=True)
            fig = px.line(df, x=df.index, y='Sales', title="Akshitha Insights: Sales Trend Graph",
            ...
            if st.button("🚀 Generate Forecast"):
                with st.spinner('Analyzing data and predicting future values...'):
                    model = sm.tsa.statespace.SARIMAX(df['Sales'], order=(int(p), int(d), int(q)), seasonal_order=seasonal_order)
                    results = model.fit()

                    st.markdown('<div class="stCard">', unsafe_allow_html=True)
                    st.markdown('<h3>🧠 Model Summary: Akshitha Predict+ Engine</h3>', unsafe_allow_html=True)
                    ...
                    st.markdown('<div class="stCard">', unsafe_allow_html=True)
                    st.markdown('<h3>📊 Future Projections</h3>', unsafe_allow_html=True)
                    fig_forecast = px.line(combined_df, 
                                           title="Forecast Powered by Akshitha Predict+",
                                           labels={'value': 'Sales/Forecast', 'index': 'Time'})
                    fig_forecast.update_traces(mode='lines+markers', hovertemplate='Date: %{x}<br>Value: %{y}')
